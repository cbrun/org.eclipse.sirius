...
/**
 * Utility class used to mark a {@link Resource} has having been migrated. A
 * {@link Resource} having such a marker *might* have been processed by a
 * migration logic during its loading hence have some changes pendings from a
 * serialization point of view.
 * 
...
public class ResourceMigrationMarker extends AdapterImpl {
    ...
    public static boolean hasMigrationMarker(Resource res) {
        Iterator<ResourceMigrationMarker> it = Iterators.filter(res.eAdapters().iterator(), ResourceMigrationMarker.class);
        return it.hasNext();
    }
}
