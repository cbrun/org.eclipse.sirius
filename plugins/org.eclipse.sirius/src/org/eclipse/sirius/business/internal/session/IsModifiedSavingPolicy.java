...
public class IsModifiedSavingPolicy extends AbstractSavingPolicy {
    ...
    public Collection<Resource> computeResourcesToSave(Set<Resource> scope, Map<?, ?> options, IProgressMonitor monitor) {
        ...
        Predicate<Resource> exists = new UnderlyingFileExists(mergedOptions);
        ...
        public boolean apply(EObject source) {
            if (!source.eIsProxy()) {
                ...
                for (EReference ref : source.eClass().getEAllReferences()) {
                     ...
                     * serialization (hence ignoring isTransient). We are
                     * ignoring derived references too as they tend to be quite
                     * intensive CPU wise, that means any dependency which is
                     * introduced *only* by a derived reference which would have
                     * been serialized will not be detected but this is very
                     * unlikely.
                     */
                    if (!ref.isTransient() && !ref.isContainment() && !ref.isDerived()) {
